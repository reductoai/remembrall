...
export async function getBasicCode(): Promise<CodeResults> {
  const renderer = await getMarkdownItInstance();
  return {
    curl: renderer.render(CurlBasic),
    js: renderer.render(JSBasic),
    nodejs: renderer.render(NodejsBasic),
    python: renderer.render(PythonBasic),
  };
}
